...
static void fluid_help(void){
    ...
    post("   (optional) -smmf: enables SMMF mode (https://bitbucket.org/agraef/pd-smmf)");
    post("   any other symbol: soundfont file to load on object creation");
    post("---------------------------------------------------------");
    post("- MESSAGES:");
    post("load /path/to/soundfont.sf2: - Load soundfont file");
    post("---------------------------------------------------------");
...
static void fluid_load(t_fluid_tilde *x, t_symbol *s, int argc, t_atom *argv){
    ...
    if(argc >= 1 && argv->a_type == A_SYMBOL){
        ...
        int fd = canvas_open(x->x_canvas, filename, ext, realdir, &realname, MAXPDSTRING, 0);
        if(fd < 0){
